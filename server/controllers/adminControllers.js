...
exports.addUser = async (req, res, next) => { //[]
    try {
        ...
        if (checkId) {
            res.status(400).json({ icon: '&#xE5CD;', style: 'error', msg: 'This ID Has Been Used Before' })
        } else if (checkEmail) {
            res.status(400).json({ icon: '&#xE5CD;', style: 'error', msg: 'This Email Has Been Used Before' })
        } else if (password.length < 8) {
            res.status(400).json({ icon: '&#xE5CD;', style: 'error', msg: 'The Password Must Be Grater Than 8 Characters' })
        } else {
            ...
            await newUser.save();
            res.status(200).json({ icon: '&#xE876;', style: 'success', msg: 'User Added Successfuly', })
        }
    } catch (error) {
        next(error)
    }
...
exports.login = async (req, res, next) => { //[]
    try {
        ...
        if (user) {
            ...
            if (!validPassword) {
                res.status(400).json({ icon: '&#xE5CD;', style: 'error', msg: 'ID or Password is not correct' });
            } else {
    ...
    try {
        ...
        if (!checkforUser) {
            ...
            User.findOneAndUpdate({ _id: id },
                ...
                (err) => {
                    if (err) {
                        res.status(404).json({ icon: '&#xE5CD;', style: 'error', msg: "Can't Update this User Information" });
                    }
                    res.status(201).json({ icon: '&#xE876;', style: 'success', msg: "User's Information Updated Successfuly" });
                });
        }
    } catch (err) {
        ...
        if (!checkforUser) {
            ...
            if (deleteUserGrade && deleteUserAttendance) {
                adminService.deleteUser(id).then((user) => {
                    if (user) {
                        res.status(201).json({ icon: '&#xE876;', style: 'success', msg: 'User Deleted Successfuly' });
                    }
                }).catch(err => {
                    console.log(err);
                    res.status(500).json({ msg: "Internal Server Error" });
        ...
        if (!checkforUser) {
            ...
            if (deleteUserGrades && deleteUserAttendance) {
                adminService.deleteUserCourse(id, courseCode).then((course) => {
                    if (course) {
                        res.status(201).json({ icon: '&#xE876;', style: 'success', msg: 'Course Deleted Successfuly from this User' });
                    }
                }).catch(err => {
                    console.log(err);
                    res.status(500).json({ msg: "Internal Server Error" });
...
exports.addCourse = async (req, res, next) => { //[]
    try {
        ...
        if (checkCourseId) {
            res.status(400).json({ icon: '&#xE5CD;', style: 'error', msg: 'This Course ID Has Been Used Before' })
        } else if (checkCourseName) {
            res.status(400).json({ icon: '&#xE5CD;', style: 'error', msg: 'This Course Name Has Been Used Before' })
        } else {
            const newCourse = new Course({ courseCode, courseName, courseDepartment, creaditHours, prerequisite });
            await newCourse.save();
            res.status(200).json({ icon: '&#xE876;', style: 'success', msg: 'Course Added Successfuly' })
        }
    } catch (error) {
        next(error)
    }
    ...
    try {
        ...
        if (!checkforcourse) {
            ...
            adminService.addCourseSemester(courseCode, semester_time).then((garde) => {
                if (garde) {
                    res.status(200).json({ icon: '&#xE876;', style: 'success', msg: "Semester Added Successfuly" });
                } else {
                    res.status(500).json({ icon: '&#xE5CD;', style: 'error', msg: "Can't Add This Semester For This Course There Is Another Opened Course Semester!" });
                }
            });
        }
    } catch (err) {
    ...
    try {
        ...
        if (!checkforcourse) {
            ...
            Course.findOneAndUpdate({ courseCode: code },
                ...
                (err) => {
                    if (err) {
                        res.status(404).json({ icon: '&#xE5CD;', style: 'error', msg: "Can't Update this Course Information" });
                    }
                    res.status(201).json({ icon: '&#xE876;', style: 'success', msg: "Course's Information Updated Successfuly" });
                });
        }
    } catch (err) {
        console.log(err.message);
        ...
        if (!checkforcourse) {
            ...
            if (deleteAllCourseGrades && deleteAllCourseAttendance) {
                adminService.deleteCourse(code).then((course) => {
                    if (course) {
                        res.status(201).json({ icon: '&#xE876;', style: 'success', msg: 'Course Deleted Successfuly' });
                    }
                }).catch(err => {
                    console.log(err);
                    res.status(500).json({ msg: "Internal Server Error" });
    ...
    try {
        ...
        if (!checkforcourse) {
            ...
            adminService.addCourseSemesterGrades(courseCode, semester_time, type, grade).then((garde) => {
                if (garde) {
                    res.status(200).json({ icon: '&#xE876;', style: 'success', msg: "Grade Added Successfuly" });
                } else {
                    res.status(500).json({ icon: '&#xE5CD;', style: 'error', msg: "Can't Add This Grade For This Course" });
                }
            });
        }
    } catch (err) {
        ...
        if (!checkforcourse) {
            ...
            if (deleteAllGradeOfThisType) {
                adminService.deleteCourseSemesterGrade(courseCode, semester_time, type).then((garde) => {
                    if (garde) {
                        res.status(200).json({ icon: '&#xE876;', style: 'success', msg: "Grade Deleted Successfuly" });
                    } else {
                        res.status(500).json({ icon: '&#xE5CD;', style: 'error', msg: "Can't Delete This Grade Form This Course" });
                    }
                });
            }
    ...
    try {
        ...
        if (!checkCourseId || !checkStudentId || !checkGradeType) {
            ...
            adminService.updateStudentGrade(studentId, courseId, semester_time, gradeType, score).then((grade) => {
                if (grade) {
                    res.status(201).json({ icon: '&#xE876;', style: 'success', msg: "Student's Grade Updated Successfuly" });
                } else {
                    res.status(404).json({ icon: '&#xE5CD;', style: 'error', msg: "Can't Update this Student's Grade Information" });
                }
            }).catch(err => {
                console.log(err);
                res.status(500).json({ msg: 'Internal Server Error' });
    ...
    try {
        ...
        if (!checkforcourse) {
            ...
            adminService.updateCourseStatus(code, status).then((change) => {
                if (change) {
                    res.status(201).json({ icon: '&#xE876;', style: 'success', msg: "Course" + " " + status + " " + "Successfuly" });
                } else {
                    res.status(404).json({ icon: '&#xE5CD;', style: 'error', msg: "Can't Change Course Status" });
                }
            }).catch(err => {
                console.log(err);
                res.status(500).json({ msg: 'Internal Server Error' });
