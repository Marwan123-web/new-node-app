    ...
    try {
        ...
        if (!checkCourseId) {
            ...
            teacherService.addTask(courseId, semester_time, taskType, taskPath).then((courseId) => {
                if (courseId) {
                    res.json({ icon: '&#xE876;', style: 'success', msg: 'Task Added Successfuly' });
                }
            }).catch(err => {
                console.log(err);
    ...
    try {
        ...
        if (!checkforcourse) {
            ...
            teacherService.deleteTask(code, semester_time, taskname).then((task) => {
                if (task) {
                    res.status(201).json({ icon: '&#xE876;', style: 'success', msg: 'Task Deleted Successfuly' });
                }
            }).catch(err => {
                console.log(err);
                res.status(500).json({ msg: "Internal Server Error" });
    ...
    try {
        ...
        if (!checkCourseId) {
            ...
            teacherService.addLecture(courseCode, semester_time, lectureNumber, lectureLocation, beacon_id).then((lecture) => {
                if (lecture) {
                    res.json({ icon: '&#xE876;', style: 'success', msg: 'Lecture Added Successfuly' });
                }
            }).catch(err => {
                console.log(err);
                res.status(500).json({ msg: 'Internal Server Error' });
    ...
    try {
        ...
        if (!checkbeacon_id) {
            ...
            teacherService.attendme(studentId, courseId, semester_time, lectureNumber, beacon_id).then((data) => {
                if (data) {
                    res.json({ icon: '&#xE876;', style: 'success', msg: 'You Attended successfuly' });
                }
                else {
                    res.status(500).json({ icon: '&#xE5CD;', style: 'error', msg: "something wrong in your data" });
                }
            });
        }
